...
apt-get -y install libcap2-bin git python3-virtualenv curl ntp build-essential screen cmake pkg-config libboost-all-dev libevent-dev libunbound-dev libminiupnpc-dev libunwind8-dev liblzma-dev libldns-dev libexpat1-dev mysql-server lmdb-utils libzmq3-dev libsodium-dev
...
Install Script:
```bash
curl -L https://raw.githubusercontent.com/blackrangersoftware/brs-nodejs-pool/master/deployment/deploy.bash | bash
```
...
You should take a look at the [wiki](https://github.com/blackrangersoftware/brs-nodejs-pool/wiki/Configuration-Details) for specific configuration settings in the system.
